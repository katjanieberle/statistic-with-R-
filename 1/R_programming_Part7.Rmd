...
## Annahmen einer klassischen Regression:

### 1. Linearität in den Parametern

- **Bedeutung**: Zusammenhang ist linear zwischen Regressoren und Regressant

- **Prüfung**: Scatterplots, Residuen vs. Fitted Plot.  

- **Verletzung**: Verzerrte Koeffizienten.

### 2. Erwartungswert der Residuen = 0

- **Bedeutung**: Keine systematische Verzerrung

- **Prüfung**: Residuen-Median $\approx 0$, Residuenplot ohne Muster  

- **Verletzung**: Bias in Intercept

### 3. Homoskedastizität** (konstante Residuen-Varianz)

- **Bedeutung**: Konstante Streuung unabhängig von x-Werten.  

- **Prüfung**: Residuen vs. Fitted (keine Trichterform), Breusch-Pagan Test.  

- **Verletzung**: Ineffiziente Schätzer, verzerrte Standardfehler.

### 4. Keine Autokorrelation der Residuen

- **Bedeutung**: Residuen unabhängig/zufällig (besonders wichtig bei Zeitreihen).  

- **Prüfung**: Durbin-Watson Test, ACF-Plot der Residuen.  

- **Verletzung**: Verzerrte Standardfehler

### 5. Exogenität** (keine simultane Kausalität)

- **Bedeutung**: Prädiktoren nicht korreliert mit Fehlertermen.  

- **Prüfung**: Keine umgekehrte Kausalität, keine Messfehler in Regressoren.  

- **Verletzung**: Endogenitätsproblem → verzerrte Koeffizienten.

### 6. Normalverteilung der Residuen (für Tests)

- **Bedeutung**: Notwendig für valide t-/F-Tests (asymptotisch entbehrlich).  

- **Prüfung**: QQ-Plot, Shapiro-Wilk Test, Histogram.  

- **Verletzung**: Größere Typ-I/II-Fehler in Tests.

### 7. Keine Multikollinearität

- **Bedeutung**: Prädiktoren linear unabhängig.  

- **Prüfung**: VIF (VarianzInflationsFaktor) < 5-10, Korrelationsmatrix

- **Verletzung**: Instabile/infinite Koeffizientenschätzungen.

---

## **Prüffolge in R** (nach `lm()`)

```{r an1, eval=FALSE}
# 1. Linearität
#plot(model, which=1)  # Residuen vs Fitted
...
# 4. Autokorrelation (Zeitreihen)
#dwtest(model)         # Durbin-Watson

# 5. Multikollinearität
#car::vif(model)       # VIF-Werte
